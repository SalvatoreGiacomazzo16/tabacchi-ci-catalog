...
const italianProductContentByFilename: Record<ProductImageFilename, ProductContent> = {
...
function buildTranslations(content: ProductContent): Record<LanguageCode, ProductTranslation> {
  return {
    it: {
      title: content.title,
...
export const products: Product[] = productImageFilenames.map((filename, index) => {
  const content = italianProductContentByFilename[filename];
  ...
  return {
    ...
    currency: 'EUR',
    translations: buildTranslations(content),
  };
});
